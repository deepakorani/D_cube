...
Agents for insilico compound screening
...
│   └── d_cube          <- Main module for the project.
│       ├── __init__.py         <- Makes d_cube a Python module.
...
Make sure to replace `d_cube` with the actual name of your
project directory.
...
```shell
vscode ➜ /workspaces/d_cube $ python --version
```
...
2. **Link Your Local Repository to GitHub:** Back in your local project,
   ...
   ```bash
   git remote add origin https://github.com/yourusername/d_cube.git
   ```
